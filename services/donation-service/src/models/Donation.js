...
function _format(row) {
  return {
    ...
    eventId: row.event_id, donorId: row.donor_id,
    donorName: row.donor_name, amount: parseFloat(row.amount) || 0,
...
const Donation = {
  async find(conditions = {}) {
    ...
    const res = await query(
      `SELECT * FROM donations WHERE ${clauses} ORDER BY created_at DESC`,
      values
    );
    return res.rows.map(_format);
  },
  ...
  async create(data) {
    const res = await query(
      ...
      [
        data.eventId, data.donorId, data.donorName,
        data.amount || 0, data.item || null, data.quantity || null,
        data.type, data.status || "completed", data.message || null
      ]
    );
